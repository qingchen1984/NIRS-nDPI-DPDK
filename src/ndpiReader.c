...
/* **************** CPM FLOWS ****************** */
typedef struct ndpi_proto_sorter {
    int  id;
    char name[16];
} ndpi_proto_sorter_t;

int cmpProto(const void *_a, const void *_b) {
    return(strcmp(((ndpi_proto_sorter_t*) _a)->name,
                                ((ndpi_proto_sorter_t*) _b)->name));
}
...
/* *********************************************** */
static int receivers_sort(void *a, void *b) {
    return(((receiver_t*) b)->num_pkts - ((receiver_t*) a)->num_pkts);
}
...
void* processing_thread(void *_thread_id) {
    ...
    while(1) {
        runPcapLoop(thread_id);
            ...
            if(playlist_fp[thread_id] == NULL) break;
            char filename[256];
